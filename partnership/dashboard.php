...
$partnershipBranchId = null;
$productBranchError = null;

if ($currentUserRole === 'partnership') {
    $partnershipBranchId = $_SESSION['branch_id'] ?? null;

    if (!$partnershipBranchId && isset($_SESSION['user_id'])) {
        ...
        if ($branchColumnCheck && $branchColumnCheck->num_rows > 0) {
            ...
            if (!empty($branchRow['branch_id'])) {
                $partnershipBranchId = $branchRow['branch_id'];
            }
        }
    }

    if (!$partnershipBranchId && isset($_SESSION['full_name'])) {
        ...
        if (!empty($managerRow['branch_id'])) {
            $partnershipBranchId = $managerRow['branch_id'];
        }
    }

    if ($partnershipBranchId) {
        $_SESSION['branch_id'] = $partnershipBranchId;
    } else {
...
if (isset($_GET['action']) && $_GET['action'] === 'get_store_details') {
    ...
    if ($store) {
        if ($currentUserRole === 'partnership') {
            if (!$partnershipBranchId || $store['branch_id'] !== $partnershipBranchId) {
                header('Content-Type: application/json');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_product'])) {
        ...
        if ($currentUserRole === 'superadmin') {
            ...
            $storeStmt->bind_param("s", $partnershipBranchId);
            $storeStmt->execute();
            ...
            $stmt->bind_param("siii", $partnershipBranchId, $storeId, $productId, $stock);
            $result = $stmt->execute();
        }
    ...
    if (isset($_POST['update_store'])) {
        ...
        if (in_array($currentUserRole, ['admin', 'superadmin'])) {
            ...
            if ($branchInfo && $partnershipBranchId && $branchInfo['branch_id'] === $partnershipBranchId) {
                $canEditStore = true;
...
if ($currentUserRole === 'partnership') {
    if ($partnershipBranchId) {
        ...
        $productStmt->bind_param("s", $partnershipBranchId);
        $productStmt->execute();
...
<head>
    ...
    <title>Dashboard partnership - Parfumé Lux</title>
        ...
        <div class="admin-user">
            ...
            <div class="admin-info">
                ...
                <span class="admin-role">
                    <?php
                    if (in_array($currentUserRole, ['admin','superadmin','partnership'])) {
                        echo 'partnership';
                    } else {
...
<div class="admin-main">
        ...
        <div class="products-table-container">
            <table class="data-table">
                ...
                <tbody id="products-tbody">
                    <?php if ($products && $products instanceof mysqli_result): ?>
                        <?php while ($product = $products->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="status-badge <?php echo ($product['status'] ?? 'active') === 'active' ? 'active' : 'inactive'; ?>">
                                        <?php echo ($product['status'] ?? 'active') === 'active' ? 'Aktif' : 'Tidak Aktif'; ?>
                                    </span>
                                </td>
                                <td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align: center; padding: 20px;">
                                <?php if ($productBranchError): ?>
                                    <p class="error"><?php echo $productBranchError; ?></p>
                                <?php else: ?>
    ...
    <!-- Scents Section (Read Only) -->
    <section id="scents-section" class="content-section">
        <div class="section-header">
            <h2>Daftar Aroma (View Only)</h2>
        </div>
    ...
    <!-- Types Section (Read Only) -->
    <section id="types-section" class="content-section">
        <div class="section-header">
            <h2>Daftar Tipe (View Only)</h2>
        </div>
    ...
    <section id="stores-section" class="content-section">
        ...
        <div class="stores-table-container">
            <table class="data-table">
                <thead><tr><th>Nama</th><th>Alamat</th><th>Telepon</th><th>Manajer</th><th>Total Produk</th><th>Aksi</th></tr></thead>
                <tbody>
                    <?php while ($store = $stores->fetch_assoc()): ?>
                        <?php
                            ...
                            if (in_array($currentUserRole, ['admin','superadmin'])) {
                                $canEditStore = true;
                            } elseif ($currentUserRole === 'partnership' && $partnershipBranchId) {
                                $canEditStore = ($store['branch_id'] ?? null) === $partnershipBranchId;
                            }
